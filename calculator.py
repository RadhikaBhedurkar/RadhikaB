...
class SC_Calculator():
    ...
    def Fact(self):
        try:
            self.result = False
            value = float(ent_field.get())
            if value < 0 or value != int(value) or value > 170:  # Factorial constraints
                self.Entry('Error')
            else:
                self.current = math.factorial(int(value))
                self.Entry(self.current)
        except (ValueError, SyntaxError):
            self.Entry('Error')
    ...
    def Tan(self):
        try:
            ...
            if abs(math.cos(math.radians(value))) < 1e-10:
                ...
                self.current = math.tan(math.radians(value))
                self.Entry(self.current)
        except (ValueError, SyntaxError):
            self.Entry('Error')
...
btn_CE = tk.Button(root, text='CE', command=lambda: sc_app.Clear_Entry(),
                   ...
                   highlightbackground='#ADD8E6', highlightthickness=2)
btn_CE.grid(row=1, column=0, sticky='n'+'s'+'e'+'w', padx=10, pady=10)

btn_AC = tk.Button(root, text='AC', command=lambda: sc_app.All_Clear(),
                   font=FONT, height=2, fg="#000000",
                   highlightbackground='#ADD8E6', highlightthickness=2)
btn_AC.grid(row=1, column=1, sticky='n'+'s'+'e'+'w', padx=10, pady=10)
...
btn_log2 = tk.Button(root, text='log₂', command=lambda: sc_app.Log_2(),
                     font=FONT, width=5, height=2, fg="#000000",
                     highlightbackground='#ADD8E6', highlightthickness=2)
btn_log2.grid(row=5, column=5, sticky='n'+'s'+'e'+'w', padx=10, pady=10)

btn_log10 = tk.Button(root, text='log₁₀', command=lambda: sc_app.Log_10(),
                      font=FONT, width=5, height=2, fg="#000000",
                      highlightbackground='#ADD8E6', highlightthickness=2)
